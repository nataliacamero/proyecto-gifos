function autocompletar(){
    ...
    inputMascota.addEventListener('input', function() {
        ...
        this.parentNode.appenChild(divList);
...
/*         //conexion a bd
        httpRequest('controller.php?tipo-mascota=' + tipoMascota, function(){
            ...
            arreglo.forEach(item => {
                if(item.substr(0, tipoMascota.length) == tipoMascota) {
                    ...
                    elementoLista.innerHTML = `<strong>${tem.substr(0, tipoMascota.length)}</strong>${item.substr(tipoMascota.length)}`;
    ...
    //aqui promesa pendiente despues de probar funcionalidad
